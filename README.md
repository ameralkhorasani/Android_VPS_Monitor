...
[![CI](https://github.com/ameralkhorasani/Android_VPS_Monitor/actions/workflows/ci.yml/badge.svg)](https://github.com/ameralkhorasani/Android_VPS_Monitor/actions/workflows/ci.yml)
...
3. **A set of power-user tools.** SSH port forwarding that survives you leaving the
   app, Docker container control, journal and file log tailing, and per-server alert
   thresholds.
...
Captured against a live VPS. Host and container names are redacted.

| Overview | Live monitor | Processes |
|:--:|:--:|:--:|
| ![Server overview](docs/screenshots/overview.png) | ![Live metrics](docs/screenshots/monitor.png) | ![Top processes](docs/screenshots/processes.png) |
| Every server at a glance, with a health score | CPU, RAM and swap straight from `/proc` | Heaviest processes, with PID and parent |

| Docker | First run |
|:--:|:--:|
| ![Docker containers](docs/screenshots/docker.png) | ![Empty state](docs/screenshots/overview-empty.png) |
| Start, stop, restart, stream logs | Nothing to show until you add a server |
...
- **Log viewing** — the app asks the server what it can actually read, offers the
  systemd journal alongside every readable file under `/var/log`, falls back to
  passwordless `sudo` where that is what it takes, and takes a custom path for
  anything it did not find.
...
│       │   │   ├── ssh/            # SSH client, metrics, docker, tunnels
│       │   │   ├── di/             # Hilt modules
│       │   │   └── ui/
...
- **Forwarded services stay private.** A forwarded service is reached only through the
  SSH tunnel, over the phone's own loopback interface; nothing is published to a public
...
[releases]: https://github.com/ameralkhorasani/Android_VPS_Monitor/releases
[gfi]: https://github.com/ameralkhorasani/Android_VPS_Monitor/labels/good%20first%20issue
[advisories]: https://github.com/ameralkhorasani/Android_VPS_Monitor/security/advisories/new
